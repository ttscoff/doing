...
Output to export format (csv|html|markdown|say|taskpaper|template|timeline)

[Default Value] None
[Must Match] (?i-mx:^(?:csv|html?|web(?:page)?|json|time(?:line)?|markdown|mk?d|gfm|task(?:paper)?|tp|template|sayit)$)
...
Output entries to format (csv|html|markdown|say|taskpaper|template|timeline)
